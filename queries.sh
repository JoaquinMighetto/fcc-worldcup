...
echo "$($PSQL "SELECT teams.name FROM games JOIN teams ON games.winner_id = teams.team_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' UNION SELECT teams.name FROM games JOIN teams ON games.opponent_id = teams.team_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY name")"
